...
# as we don't know the number of elements, we have to use *args method
# in order to use them all

def is_divisible(*args):
    for element in args:
        # we have to compare the first element of the list
        # of arguments with all the elements from the list
        if list(args)[0] % element != 0:
            return False
